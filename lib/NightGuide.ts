...
// Module-level conversation history used as a fallback/persistent in-memory store
...
export async function askAI(prompt: string, history: Content[] = []): Promise<{ text: string, newHistory: Content[] }> {
  ...
  for (let i = 0; i < MODEL_PRIORITY.length; i++) {
    ...
    try {
      ...
      // update module-level conversation history
      ...
      if (isLastModel || !isRetryable) {
        return { text: handleError(err), newHistory: history };
      }
