...
import { FaShareAlt, FaGlobe, FaLock, FaLink } from "react-icons/fa";
...
export default function PlaylistCard({
  ...
  return (
    <Link
      href={href}
      className="group relative w-full h-36 md:h-40 rounded-xl overflow-hidden border border-neutral-800 hover:border-red-600/50 bg-neutral-900/40 backdrop-blur-md transition-all duration-300 flex hover:shadow-[0_0_20px_rgba(220,38,38,0.15)] shadow-xl"
    >
      {/* Decorative Red Accent Background Hover */}
      <div className="absolute inset-0 bg-gradient-to-r from-red-600/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-0" />

      {/* Card Content Wrapper */}
      <div className="relative z-10 flex flex-row items-center w-full h-full p-3 gap-4">
        {/* Thumbnail Stack Layout (Left) */}
        <div className="relative h-full aspect-2/3 shrink-0 rounded-lg overflow-hidden border border-white/5 bg-neutral-950/85 flex flex-col items-center justify-center shadow-inner group-hover:border-red-600/30 transition-colors duration-300">
