...
async function fetchMovieDetails(movieID) {
  try {
    const response = await fetch(
      `${base_url}/movie/${movieID}?api_key=${api_key}&language=en-US&append_to_response=credits,similar`
    );
...
function displayMovieDetails(movie) {
  ...
  document.getElementById("movie-release-date").textContent = formatReleaseDate(movie.release_date);
  document.getElementById("movie-runtime").textContent = formatRuntime(movie.runtime);
  document.getElementById("movie-rating").textContent = `${movie.vote_average.toFixed(1)}/10 ${generateStarRating(movie.vote_average)}`;
  ...
  movies.forEach(movie => {
    ...
    movieCard.innerHTML = `
      ...
      <div class="similar-movie-info">
        ...
        <div class="similar-movie-rating">${movie.vote_average.toFixed(1)}/10</div>
      </div>
    `;
